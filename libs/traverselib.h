...
public:
	...
	void insert( scene::Node& node ) override {
		ASSERT_MESSAGE( (intptr_t)&node != 0, "TraversableNodeSet::insert: sanity check failed" );
		m_undo.save();
	...
	void erase( scene::Node& node ) override {
		ASSERT_MESSAGE( (intptr_t)&node != 0, "TraversableNodeSet::erase: sanity check failed" );
		m_undo.save();
